...
MonitoringTabsModule.prototype.loadGridElementLabelBarEvents = function(moduleContainer) {
    ...
    // minimise maximise on minus/plus button clicked 
...
MonitoringTabsModule.prototype.addMonitoringModule = function(channelName) {
    ...
    $(_this.lesElementsGridColumn()).append(monitoringOuterContainer);

    // configure jqueryUi grid elements behavior
    $(".sortable_grid_column").sortable({
        // tolerance: "pointer",www
        revert: true,
...
MonitoringTabsModule.prototype.lesElementsGridColumn = function() {
    var _this = this;
    ...
    // determine column elements quantity
    var firstColLen = $(firstCol).find('.grid_element').not('.grid_element.template').length; 
    var secondColLen = $(secondCol).find('.grid_element').not('.grid_element.template').length;

    // select column with less grid elements quantity
    if (firstColLen > secondColLen) {
        return secondCol;
    }
